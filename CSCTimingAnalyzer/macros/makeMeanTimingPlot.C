...
void makeMeanTimingPlot (std::string fname, bool byStation, bool no_legend = false)
{    
    ...
    h1->GetYaxis()->SetTitle("mean cathode hit time (ns)");
    h1->SetTitle("Mean and RMS of cathode hit time for each ring of CSC chambers");
    h1->SetTitleFont(42);
    ...
    for (auto hist : *hlist)
    {
        ...
        if (byStation)
        {
            ...
            double mean = ((TH1*)obj)->GetMean() * 50.;
